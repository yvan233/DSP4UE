...
marker_indices = [0, 400, 800, 1200, 1500]
...
for velocity_x in velocities_x:
    axs[0].plot(times, velocity_x, alpha=0.7)
...
for velocity_y in velocities_y:
    axs[1].plot(times, velocity_y, alpha=0.7)
axs[1].set_title('Y Velocity over Time')
axs[1].set_xlabel('Time (s)')
axs[1].set_ylabel('Y Velocity (m/s)')

# 绘制代数连通度随时间变化
axs[2].plot(marker_indices, algebraic_connectivity, '-o', color='red')
...
plt.tight_layout()
plt.savefig('velocity_and_connectivity.png', dpi=300)
plt.show()
