...
def taskFunction(self:Task, id, nbrDirection, datalist):
    ...
    if uavid == 0:
        ...
        while True:
            ...
            # 计算所有无人机的避障速度
            avoid_acc_dict = avoid_control(uav_state)
            # 广播到所有无人机
            sendmessage = [state, avoid_acc_dict]
        ...
        while True:
            ...
            avoid_acc_dict = nbrData[0][1]
            avoid_acc = avoid_acc_dict[name]
            follower_state = state
            # 计算偏移量
            ax, ay, az = follower_control_acc(leader_state, follower_state, target_leader_state, target_follower_state, avoid_acc)
            print_iter += 1
            if print_iter % 10 == 0:
                print_iter = 0
