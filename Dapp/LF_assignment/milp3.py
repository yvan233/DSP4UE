import pulp as pl
import numpy as np
# https://blog.csdn.net/weixin_46967285/article/details/128103935
...
# 创建变量
t = pl.LpVariable.dicts("t", (size, size),0,None,pl.LpContinuous)
c = pl.LpVariable.dicts("c", (size, size),0,1,pl.LpInteger)

# 添加目标函数
# prob += pl.lpSum(cost_matrix[i][j] * c[i][j] for i in size for j in size)
prob += pl.lpSum(t[i][j] for i in size for j in size)
# 添加约束条件

for i in size:
    for j in size:
        prob += t[i][j] >= cost_matrix[i][j] * c[i][j]
for i in size:
...
for i in size:
    for j in size:
        ...
        cost_max = max(cost_max,cost_matrix[i][j] * c[i][j].varValue)
print(cost_sum,cost_max)
